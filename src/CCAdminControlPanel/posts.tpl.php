...
<?php if($posts != null):?>
  <?php foreach($posts as $val):?>
    ...
    <p class='smaller-text silent'><a href='<?php echo create_url("content/edit/{$val['id']}")?>'>edit</a>  <a href='<?php echo create_url("acp/deletepost/{$val['id']}")?>'>delete</a></p>
    <h3><br />Comments:</h3>
    <?php foreach($comments as $com):?>
		<?php if($com['postId'] == $val['id']):?>
	  		...
	  		<div class = 'comment'>
			  	...
			  	<p class='smaller-text silent'><a href='<?php echo create_url("acp/deletecommentpost/{$com['id']}")?>'>delete</a></p>
			</div>
		<?php endif; ?>
	  <?php endforeach; ?>
